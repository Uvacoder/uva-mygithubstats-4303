...
export default function User() {
  ...
  const mostStarredRepos = userUtil.getMostStarredRepos(repositories).slice(0, internals.MAX_STARRED_REPOS_TO_SHOW);
  const mostForkedRepos = userUtil.getMostForkedRepos(repositories).slice(0, internals.MAX_FORKED_REPOS_TO_SHOW);
  const forksPerLanguage = userUtil.getForksPerLanguage(repositories, internals.MAX_FORKS_PER_LANG_TO_SHOW);
  const starsPerLanguage = userUtil.getStarsPerLanguage(repositories, internals.MAX_STARS_PER_LANG_TO_SHOW);
  ...
  const archiveReposCount = repositories.filter(r => r.node.isArchived).length;
  const mirrorReposCount = repositories.filter(r => r.node.isMirror).length;
  const sourceRepoCount = repositories.length - forkReposCount - archiveReposCount - mirrorReposCount;
        ...
        <div className='mb2'>
          <ul className='icon-list'>
            <li>
              ...
              <span className='fw600'>
                {prettyNumber(repositories.length)}
              </span> repositories
              {' '}
              <small>
                ({sourceRepoCount} Sources)
              </small>
      ...
      <div>
        ...
        <section className='mb3'>
          <h2 className='fs-lg fw500 mb1'>
            Recently contributed to {user.repositoriesContributedTo.totalCount} repositories
          </h2>
          {Boolean(user.repositoriesContributedTo.totalCount) && (
            <>
              {user.repositoriesContributedTo.nodes.some(r => r.isInOrganization) && (
                <div className='flex aic mb1'>
                  <p className='tertiary-text mr05'>
                    Organizations
                  </p>
                  <ul className='clean-list flex'>
                    {[...new Map(user.repositoriesContributedTo.nodes
        ...
        <section className='mb3'>
          ...
          <div className='paper'>
            <div className='mb'>
              <p className='fw500 mb05'>
                Primary user repository languages
              </p>
              <div className='tertiary-text'>
                <LanguagesChart data={languagesPerRepo} colors={languageColors} height={4} />
              </div>
            </div>

            <div className='paper-divider--h'/>

            <div className='grid' style={{'--columns': 2, '--gap': '1rem 3rem'}}>
              <div>
                <p className='fw500 mb1'>
                  Stars per language (top 5)
                </p>
                <PieChart
