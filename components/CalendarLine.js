...
export default function CalendarLine({
  ...
  const monthsLabel = months
    .map((m) => {
      return [m.name, new Array(m.totalWeeks - 1).fill()].flat();
    })
    .flat();
  const totalWeeks = weeks.length;
  const max = Math.max(...contributionsPerWeek.map((w) => w.total));

  let polygonPoints = `M${-lineWidth},${
    ...
    <>
      ...
      >
        {new Array(5).fill(0).map((_, i, arr) => {
          ...
          return (
            <g key={i}>
              <line x1={0} x2={graphWidth} y1={y} y2={y} />
              <text x={graphWidth + 4} y={y} dominantBaseline="middle">
